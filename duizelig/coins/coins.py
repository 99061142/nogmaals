...
# structure of program: loop through the problem if the user has not paid the total price
...
while change > 0 and coinValue > 0: 
    ...
    if nrCoins > 0: # If the user can pay with a coin that is in the coin list
        ...
        nrCoinsReturned = int(nrCoinsReturned) # Change the input what the user has paid with the specific value to a number

        change -= nrCoinsReturned * coinValue # The change gets decreased with the value the user has paid\


    coinArray = [500, 300, 200, 100, 50, 20, 10, 5, 2, 1, 0] # Array with all the coin values

    # Change the value of the coin, to the next coin value in the coin array
